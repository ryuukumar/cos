 ...
 * byte string pointed to by s (each character interpreted as unsigned char). The terminating null
 * character is considered to be a part of the string and can be found if searching for '\0'.
 * @param s string to search in
 * @param c char to search for
 * @return pointer to the found character in s, or null pointer if no such character is found.
 */
char* kstrrchr (char* s, int c) {
	size_t len = kstrlen (s), i = len + 1;
	while (i-- > 0)
